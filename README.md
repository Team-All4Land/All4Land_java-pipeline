...
08.07 산출물을 실제로 밀어 넣으면 첫 질의가 **4건**을 돌려줍니다 — 영광군청 465/466,
양양군청(게시완료) 0/1, 그리고 기관을 못 붙인 전자관보 2기관(82/0·77/0)입니다.
...
**① 전자관보 159건은 기관이 붙지 않습니다.** 검증요약은 `72_1 전자관보_농림축산식품부`(82건)와
`72_2 전자관보_새만금개발청`(77건)으로 갈라 놓았는데, 수집결과의 지자체명은 둘 다 그냥
`전자관보`입니다. 둘을 가르는 유일한 근거가 비고(`제목이 "농림축산식품부고시"로 시작함`)인데
비고는 적재하지 않기로 했고, 제목 칸 자체에는 그 접두어가 없습니다(159건 모두
`공유수면 점용·사용…`으로 시작합니다). 그래서 이 159건은 `AGNCY_SN`이 NULL로 들어갑니다.
...
행수와 대조하라고 담는 이유입니다. 08.07 산출물에서는 영광군청 465/466, 양양군청_이전공고
0/1이 어긋났습니다(전자관보 2기관은 ①번 때문에 대조 불가).
...
**전자관보 159건만은 기관이 붙지 않습니다.** 검증요약은 `72_1`·`72_2` 두 기관으로 갈라 놓았는데
수집결과의 지자체명이 둘 다 `전자관보`라 어느 쪽인지 가릴 근거가 없습니다
