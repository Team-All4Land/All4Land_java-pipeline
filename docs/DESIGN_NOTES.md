...
분류어가 지시하는 도메인과 DDL 선언이 같은지, 감사 컬럼이 면제받지 않은 전 테이블에 있는지
(면제를 선언해 놓고 컬럼을 남겨 뒀는지까지 양방향으로), 예약어를 쓰지 않았는지, **테이블이 업무코드로 시작하는지**, **제약조건·인덱스가 표준 형식인지**,
...
사유를 기록하고, **항목값 적재만** 건너뜁니다. 파일을 통째로 빼는 것이 아닙니다 —
첨부 행(`OS_ATCH_FILE_DTL`)도 이미지(`OS_ATCH_IMG_DTL`)도 그대로 들어가고,
`OS_NOTI_ITEM_VAL_DTL`·`OS_NOTI_LBL_VAL_DTL`만 비웁니다. 그래서 처리상태코드의 이름이
그냥 "적재제외"가 아니라 **항목값 적재제외**입니다 — 짧은 이름 하나 때문에 "아예 안 넣는다"로
읽혔습니다. `raw.json`과 `schema.json`은 그대로 남으므로 판단이 틀렸으면 `load`로 다시
넣으면 됩니다.

```
[항목값 적재제외] 전자문서 이용안내.pdf: 본문 8,355자 (임계 5,000자 초과) — 안내문류로 보고 항목값 적재만 건너뜁니다(첨부·이미지는 적재)
DB 적재: 첨부 7건, 항목값 9행, 이미지 3행 (항목값 적재제외 1건)
```
...
이미 적재한 뒤라면 `OS_ATCH_FILE_DTL.EXCL_RSN_CTNT`에도 그 수치가 남아 있습니다 — 본문
글자 수 컬럼을 따로 두지 않았으므로 **DB 안에서 그 값을 아는 자리는 거기뿐**입니다. 다만
제외된 행만 갖고 있으니, 임계 아래쪽 분포까지 보려면 `schema.json`을 봐야 합니다.
