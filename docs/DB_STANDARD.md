...
| `D_CD` | 코드 | `VARCHAR(30)` | — | 표준코드 허용값. 최장값이 MIXED_CHG_PRMSN_CNSLT_APRV(26자)라 30으로 잡았다 |
...
| 고시공고게시물 | `NOTI_BAS` | `NOTI_SN` | 게시물. 같은 게시물의 첨부를 묶으며 담당부서·담당자·전화번호·고시공고번호는 크롤러가 수집한다. 첨부 추출… |
...
| 담당부서명 | `CHRG_DEPT_NM` | 담당 + 부서 + 명 | `D_NM` | — | 크롤러가 수집한 고시·공고 게시물 담당부서명 |
| 담당자명 | `CHRGR_NM` | 담당자 + 명 | `D_NM` | — | 크롤러가 수집한 고시·공고 게시물 담당자명 |
| 전화번호 | `TEL_NO` | 전화 + 번호 | `D_NO` | — | 크롤러가 수집한 고시·공고 게시물 담당 전화번호 |
...
| 첨부파일경로 | `ATCH_FILE_PATH` | 첨부 + 파일 + 경로 | `D_PATH` | — | 입력 첨부파일의 정규화된 절대경로 |
...
| 고시공고번호 | `NOTI_NO` | 고시공고 + 번호 | `D_NO` | — | 문서 본문의 고시·공고번호("고시 제2026-47호"). 게시물 일련번호와도, 처분받은 허가번호와도 다른 … |
