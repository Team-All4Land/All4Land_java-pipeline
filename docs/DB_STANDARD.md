...
| 실패메시지내용 | `FAIL_MSG_CTNT` | 실패 + 메시지 + 내용 | `D_CTNT` | — | 사람이 읽는 실패 설명 원문. 같은 실패종류 여러 건 중 무엇이 진짜 손상이고 무엇이 우리 파서 한계인지는… |
| 적재제외사유내용 | `EXCL_RSN_CTNT` | 제외 + 사유 + 내용 | `D_CTNT` | — | 항목값 적재를 건너뛴 사유이자 그 판정의 측정값 — "본문 8,355자 (임계 5,000자 초과)" 형태다… |
...
첨부파일 1건의 처리 결과. SKIP은 파일을 통째로 빼는 것이 아니다 — 첨부 행도 이미지도 들어가고 항목값·라벨값만 빠진다
...
파이프라인 단계. 콘솔에는 한글 표시명이 나가고 DB에는 코드가 들어간다. 자체 실패를 정의하지 않는 구간은 값을 두지 않는다 — 실측 0건인 값이 섞이면 이 표를 믿을 수 없게 된다. EXTC가 추출부터 매핑까지를 덮고, SAVE는 중간 산출물 파일 저장이며 DB 적재는 LOAD다
