...
| 크롤로그 | `CRWL_LOG_DTL` | `CRWL_LOG_SN` | 크롤 실행 1건의 완료·실패 기록. 게시물이 하나도 안 생긴 기관을 설명하는 유일한 자리라 기관 FK를 두… |
...
크롤 로그 한 줄의 결과. 완료 표시도 행으로 남긴다 — 실패만 적재하면 "돌긴 돌았는데 아무것도 못 건진 실행"과 "아예 안 돈 실행"이 구분되지 않는다
