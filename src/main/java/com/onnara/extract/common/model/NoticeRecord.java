 ...
 *
 * @param fields canonical 필드명 → 값. 사전 선언 순서로 정렬돼 있어 JSON 산출물이 안정적이다
...
public record NoticeRecord(
    ...
    /** canonical 필드명으로 값을 읽는다(없으면 null). */
    ...
    // ── 편의 접근자 ──────────────────────────────────────────────────
    // 문서 메타(attachments 컬럼이 되는 5개)와 자주 쓰는 처분 값들.

    /** 고시·공고를 발령한 행정기관명. */
    public String agency() {
        return fields.get("agency");
    }

    /** 고시·공고의 문서번호. */
    public String noticeNo() {
        return fields.get("notice_no");
    }

    /** 고시·공고가 공표된 날짜(ISO). */
    public String noticeDate() {
        return fields.get("notice_date");
    }
    ...
    public static final class Builder {
        ...
        public NoticeRecord build() {
            ...
            for (String canonical : Synonyms.fieldOrder()) {
                String value = fields.get(canonical);
                if (value != null) {
