 ...
 *
 * @param agenciesUpserted OS_INSTT_BAS에 반영한 기관 수 — 첨부가 0건인 폴더도 세므로
 ...
 * @param recordsInserted  삽입된 OS_NOTI_ITEM_VAL_DTL 행 수(= 표준항목 값 총합)
 * @param labelsInserted   삽입된 OS_NOTI_LBL_VAL_DTL 행 수(= 표준항목으로 매핑되지 못한 값 총합).
 *                         이 수가 늘고 있다면 사전에 구멍이 있다는 뜻이므로 요약에 함께 낸다
 * @param imagesInserted   삽입된 OS_ATCH_IMG_DTL 행 수(= 경로가 있는 이미지 총합)
 */
